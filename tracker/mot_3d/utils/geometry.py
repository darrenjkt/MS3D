...
__all__ = ['pc_in_box', 'downsample', 'pc_in_box_2D',
           ...
           'get_abs_angle_diff','get_directed_angle_diff']

def make_vector(angle):    
    return np.hstack([np.cos(angle).reshape(-1,1), 
                      np.sin(angle).reshape(-1,1)])

def get_mean_rotation(angles):
    """
    Computes mean rotation in vector space due to wrap-around of angles
    """
    mean_vec = np.mean(make_vector(angles), axis=0)
...
def get_directed_angle_diff(a1, a2):
    """
    Gives directed angle such that a1 + angle_diff = a2. 
    Note that taking the abs of this can be different to "get_abs_angle_diff" 
    e.g. 328 deg angle diff from this function would be 32 deg from "get_abs_angle_diff" 
    """
    v1 = np.array([np.cos(a1), np.sin(a1)])
    v2 = np.array([np.cos(a2), np.sin(a2)])
    angle_diff = np.arctan2(v1[1],v1[0]) - np.arctan2(v2[1],v2[0])
    return angle_diff
