...
class DataProcessor(object):
    ...
    def sample_points(self, data_dict=None, config=None):
        ...
        if num_points < len(points):
            ...
            pts_near_flag = pts_depth < config.PTS_NEAR_FLAG
        ...
        # If less point cloud points than how much we want to sample, take all the points, and add some duplicate points to it
        else:
            choice = np.arange(0, len(points), dtype=np.int32)
            if num_points > len(points):
                try:
