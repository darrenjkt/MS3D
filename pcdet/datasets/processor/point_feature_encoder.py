...
class PointFeatureEncoder(object):
    def __init__(self, config, point_cloud_range=None):
        ...
        assert list(self.point_encoding_config.src_feature_list[0:3]) == ['x', 'y', 'z'], f'instead received: {list(self.point_encoding_config.src_feature_list[0:3])}'
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        assert points.shape[-1] == len(self.src_feature_list), f'points.shape[-1]={points.shape[-1]}, len(self.src_feature_list)={len(self.src_feature_list)}'
