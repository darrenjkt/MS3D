...
def global_translation(gt_boxes, points, trans_range, return_trans=False, translation=None):
    """
    Args:
        gt_boxes: (N, 7), [x, y, z, dx, dy, dz, heading, [vx], [vy]]
        points: (M, 3 + C),
        trans_range: [x_range, y_range, z_range] e.g. trans_range=[50,50,2]
        translation: [x_trans,y_trans,z_trans]
    Returns:
    """
    if translation is None:
        enable_x = np.random.choice([False, True], replace=False, p=[0.5, 0.5])
        enable_y = np.random.choice([False, True], replace=False, p=[0.5, 0.5])
        enable_z = np.random.choice([False, True], replace=False, p=[0.5, 0.5])
        x_trans = np.random.uniform(-trans_range[0], trans_range[0]) if enable_x else 0.0
        y_trans = np.random.uniform(-trans_range[1], trans_range[1]) if enable_y else 0.0
        z_trans = np.random.uniform(-trans_range[2], trans_range[2]) if enable_z else 0.0
        translation = np.array([x_trans, y_trans, z_trans])
    else:
        translation = np.array(translation)

    points[:,:3] += translation
    gt_boxes[:,:3] += translation
    if return_trans:
        return gt_boxes, points, translation
    return gt_boxes, points
...
def scale_pre_object(gt_boxes, points, gt_boxes_mask, scale_perturb, num_try=50):
    ...
    for k in range(num_boxes):
        ...
        obj_center, lwh, ry = gt_boxes[k, 0:3], gt_boxes[k, 3:6], gt_boxes[k, 6].reshape(1)
...
def normalize_object_size(boxes, points, boxes_mask, size_res):
    ...
    for k in range(boxes.shape[0]):
        ...
        obj_center, lwh, ry = boxes[k, 0:3], boxes[k, 3:6], boxes[k, 6].reshape(1)
        obj_points[:, 0:3] -= obj_center
        obj_points = common_utils.rotate_points_along_z(np.expand_dims(obj_points, axis=0), -ry).squeeze(0)
...
def cart2sph(points):
    ...
    sph_pts = np.concatenate((r[:,np.newaxis],azimuth[:,np.newaxis], elevation[:,np.newaxis]), axis=1)    
    if points.shape[1] > 3:
        sph_pts = np.hstack([sph_pts, points[:,3:]])
    return sph_pts
...
def sph2cart(points):
    ...
    cart_pts = np.concatenate((x[:,np.newaxis],y[:,np.newaxis],z[:,np.newaxis]), axis=1)
    if points.shape[1] > 3:
        cart_pts = np.hstack([cart_pts, points[:,3:]])
    return cart_pts

def local_ring_dropout(gt_boxes, points, min_pts, max_sel_n_hpts_1_2_ring=30):
    """
    Bins the spherical coordinates then selects every Nth bin randomly (aka. lidar ring)
    Make sure the points and gt_boxes are in their sensor_frame i.e. undo the SHIFT_COOR for this function
    """
    # min_score=min_score_pts[0] # gt has no score
    min_in_pts=min_pts[0]
    min_out_pts=min_pts[1]
    
    for box in gt_boxes:
        points_in_box, points_mask = get_points_in_box(points, box)
        enable = False
        if (points_in_box.shape[0] > min_in_pts):
            enable = np.random.choice([False, True], replace=False, p=[0.5, 0.5])

        aug_obj_pts = None
        if enable:
            sph_pts = cart2sph(points_in_box)
            ...
            # Skip at most num_rings*0.3 since after that, most of the values only lead to giving 3-5 rings. 
            ...
            if onetwo_ring and len(choose_rings) > 2:
                ...
                aug_obj_pts = sph2cart(sph_pts[onetwo_mask][np.random.randint(0,min(counts))::sel_n_hpts])
                if len(aug_obj_pts) < min_out_pts:
                    aug_obj_pts = out
            else:
                if len(out) > min_out_pts:
                    aug_obj_pts = out
                else:
                    aug_obj_pts = points_in_box
        
        if aug_obj_pts is not None:
            points = np.vstack([points[~points_mask], aug_obj_pts])
    return points