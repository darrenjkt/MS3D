...
class Tracklet(object):
    ...
    objectType = None
    size = None  # len-3 float array: (length, width, height)
    firstFrame = None
    ...
    def consolidate(self):
        for idx in range(self.nFrames):
            bbox = np.hstack([(self.trans[idx] + [0,0,self.size[2]/2]), self.size, self.rots[idx,2]])
            self.boxes.append(bbox)
            self.sample_indices.append(self.firstFrame + idx)
