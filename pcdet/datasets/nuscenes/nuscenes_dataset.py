...
class NuScenesDataset(DatasetTemplate):
    ...
    def include_nuscenes_data(self):
        ...
        if self.custom_train_scenes is not None:      
            def check_custom_scenes(info):
                return info['scene_name'] in self.custom_train_scenes
            nuscenes_infos = list(filter(check_custom_scenes, nuscenes_infos))

        self.infos.extend(nuscenes_infos)
        for idx, data in enumerate(self.infos):
            self.frameid_to_idx[Path(data['lidar_path']).stem] = idx
    ...
    def get_lidar_with_sweeps(self, index, max_sweeps=1):
        ...
        for k in np.random.choice(max_sweeps, max_sweeps - 1, replace=False): # Randomizing the order instead of sequentially; seems to help performance a bit
