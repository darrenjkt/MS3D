...
class OpenPCDetWaymoDetectionMetricsEstimator(tf.test.TestCase):
    ...
    def build_config(self, **kwargs):
        ...
        breakdown_generator_ids = kwargs['breakdown_generator_ids'] if 'breakdown_generator_ids' in kwargs else 'RANGE'
        ...
        iou_thresholds: 0.5
        box_type: TYPE_3D
        """ % (breakdown_generator_ids, veh_iou_threshold, ped_iou_threshold)
        # IOU thresholds are UNKNOWN=0.0, VEH=0.7, PED=0.5, SIGN=0.5, CYC=0.5
    ...
    def build_graph(self, graph, **kwargs):
    ...
    def waymo_evaluation(self, prediction_infos, gt_infos, class_name, distance_thresh=100, fake_gt_infos=True, **kwargs):
        ...
        graph = tf.Graph()
        metrics = self.build_graph(graph, **kwargs)
