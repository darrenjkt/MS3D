...
class WaymoDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if self.dataset_cfg.get('USE_PREDBOX', False):
            self.pred_boxes_dict = self.load_pred_boxes_to_dict(
                pred_boxes_path=self.dataset_cfg.ROI_BOXES_PATH[self.split]
            )
        else:
    ...
    def include_waymo_data(self):
        ...
        num_skipped_infos = 0
        if self.custom_train_seq is not None:
            self.sample_sequence_list = self.custom_train_seq
        for k in range(len(self.sample_sequence_list)):
            ...
            seq_name_to_len[infos[0]['point_cloud']['lidar_sequence']] = len(infos)
        ...
        # If we set SAMPLED_INTERVAL=5, we'll be getting the dataset at 2Hz intervals
    ...
    def get_sequence_data(self, info, points, sequence_name, sample_idx, sequence_cfg, load_pred_boxes=False):
        ...
        def load_pred_boxes_from_dict(sequence_name, sample_idx):
            """
            boxes: (N, 11)  [x, y, z, dx, dy, dz, yaw, vx, vy, score, label]
            """
        ...
        points = remove_ego_points(points, center_radius=1.0)
