...
def fill_trainval_infos(data_path, lyft, train_scenes, val_scenes, test=False, max_sweeps=10):
    train_lyft_infos = []
    val_lyft_infos = []
    progress_bar = tqdm.tqdm(total=len(lyft.scene), desc='create_info', dynamic_ncols=True)
    ...
    prev_scene_token = ''
    for index, scene in enumerate(lyft.scene):
        progress_bar.update()

        first_tok = scene['first_sample_token']
        sample = lyft.get('sample', first_tok)
        while True:
            ...
            if sample['next'] == '':
                break
            sample = lyft.get('sample', sample['next'])
