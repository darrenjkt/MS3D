...
def corners_3d_to_bboxes(corners):
    """
    Assumes object length is from idx 1 to idx 2 of box corners for heading calculation
    Args:
        corners: (N, 8, 3) N box corners
    """
    centroids = np.mean(corners, axis=1)
    pt_diff = corners[:,1,:] - corners[:,2,:]
    unit_vecs = pt_diff/np.linalg.norm(pt_diff)
    rys = np.arctan2(unit_vecs[:,1], unit_vecs[:,0])
    corners -= centroids[..., np.newaxis].reshape(-1,1,3)
    cn_corners = common_utils.rotate_points_along_z(corners, -rys)
    
    dims = np.max(cn_corners, axis=1) - np.min(cn_corners, axis=1)
    opd_boxes = np.hstack([centroids, dims, rys.reshape(-1,1)])
    return opd_boxes        
