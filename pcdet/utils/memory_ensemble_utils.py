...
def simplified_cons_ensemble(gt_infos_a, gt_infos_b, st_cfg):
    memory_ensemble_cfg = st_cfg.MEMORY_ENSEMBLE
...
def consistency_ensemble(gt_infos_a, gt_infos_b, st_cfg):
    """
    Args:
        gt_infos_a:
    ...
    """    
    MY UNDERSTANDING:
    Ignore boxes (but in gt_boxes) are labelled with neg class
        
    When there's a new, better box, the new box has positive class, so it'll replace the
    old neg class box
    """
    iou_mask = (ious >= memory_ensemble_cfg.IOU_THRESH)
    ...
    """
    MY UNDERSTANDING:
    For already ignored boxes, there'll already be a neg class, so they just 
    increment mem_counter
    
    new_gt_box[ignore_mask, 7] = -1 --> for the positive categories, it can also 
    be unmatched. So it may start off as 1, but it becomes ignored (-1) in training
    if there are no matches over time
    
    If any boxes have mem_counter > RM_THRESH=3, they discard it        
    """
    ...
    """
    MY UNDERSTANDING:
    For new boxes, if there is no match (ious_b2a < memory_ensemble_cfg.IOU_THRESH)
    for any current frame pred box in historical box, then add it into the pseudo-labels
    
    NEG_THRESH < score < POS_THRESH filtering is done in "save_pseudo_label_batch". So in 
    that case, even if score_new=0.4 replaces a score_old=0.3, the new gt_box still has a 
    neg class

    Comment this section out if don't want to add new boxes
    """
...
def memory_ensemble(gt_infos_a, gt_infos_b, memory_ensemble_cfg, ensemble_func):
    ...
    if n_classes == 0:
        ...
        # Multi-class has not been implemented
        print(f'classes_a: {classes_a}, classes_b: {classes_b}')
