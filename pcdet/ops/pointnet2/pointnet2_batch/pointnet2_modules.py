        ...
        if ctr_xyz is None:
            ...
            for i in range(len(self.sample_type_list)):
                ...
                if sample_range == -1: #全部
                    ...
                    feature_tmp = features.transpose(1, 2)[:, last_sample_end_index:, :].contiguous() if features is not None else None   
                    ...
                    feature_tmp = features.transpose(1, 2)[:, last_sample_end_index:sample_range, :] if features is not None else None   
                ...
                if xyz_tmp.shape[1] <= npoint: # No downsampling
                    ...
                    features_SSD = torch.cat([xyz_tmp, feature_tmp], dim=-1) if feature_tmp is not None else xyz_tmp
